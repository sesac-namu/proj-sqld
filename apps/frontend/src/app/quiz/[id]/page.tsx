...
const MOCK_QUESTIONS = [
  ...
  },
  {
    ...
    answer: 'CREATE',
    explanation: 'CREATE, ALTER, DROP 등은 테이블 구조나 제약조건을 정의하는 DDL 명령어입니다.',
  },
  // ... 더 많은 문제들
];
...
export default function QuizPage() {
  ...
  if (!quizData) {
    return (
      <div className='text-center mt-20'>
        ...
        <p className='text-sm text-slate-400 mt-2'>또는 해당 ID의 퀴즈를 찾을 수 없습니다.</p>
        <Link href='/quiz' className='mt-4 inline-block text-blue-500 hover:underline'>
          문제 목록으로 돌아가기
        </Link>
      </div>
    );
    ...
    return (
      ...
      <div className='max-w-lg mx-auto mt-10 bg-white p-8 rounded-lg shadow-xl text-center'>
        ...
        <p className='text-xl mb-4'>
          총 <span className='font-semibold'>{quizData.questions.length}</span>문제 중
          <span className='font-semibold text-green-600'> {score}</span>문제를 맞혔습니다.
        </p>
  ...
  return (
    <div className='max-w-2xl mx-auto mt-8 bg-white p-6 md:p-8 rounded-lg shadow-xl'>
      <h1 className='text-2xl font-bold text-slate-700 mb-2'>{quizData.title}</h1>
        ...
        <div className='space-y-3'>
          {currentQuestion.options.map((option, index) => (
            <button
              ...
              disabled={showAnswer}
              className={`
                block w-full text-left p-3 rounded-md border transition-all
                ${
      ...
      <div className='mt-8 flex justify-between items-center'>
        <button
          ...
          className='bg-green-500 hover:bg-green-600 text-white font-semibold py-2 px-6 rounded-md transition-colors disabled:bg-slate-300'
